  ...
  if ($type == "2"){
    ...
    if ($sql){
      if (mysqli_num_rows($sql)==0){
        ...
        $sql1=$conn->query("INSERT into tb_audittrail (logid, userid, day, login) VALUES('null','" . $row['UserID'] . "', '" .
          date("Y-m-d") . "', '" . date('H:i:s') . "')");
        if ($sql1){
          $sql2 = $conn->query("SELECT * FROM tb_audittrail WHERE userid='" . $row['UserID'] . "' ORDER BY logid DESC");
    ...
    if ($sql){
      if (mysqli_num_rows($sql)==0){
        ...
        $sql1=$conn->query("INSERT into tb_audittrail (logid, userid, day, login) VALUES('null','" . $row['UserID'] . "', '" .
          date("Y-m-d") . "', '" . date('H:i:s') . "')");
        if ($sql1){
          $sql2 = $conn->query("SELECT * FROM tb_audittrail WHERE userid='" . $row['UserID'] . "' ORDER BY logid");
          $row1 = $sql2->fetch_assoc();
          $sql3 = $conn->query("INSERT into tb_module (moduleID, module, loginId) VALUES ('null', 'home', '" . $row1['logid'] . "')");
          $_SESSION['logid'] = $row1['logid'];
          $_SESSION['module'] = "home";
